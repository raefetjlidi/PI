...
def index():
    ...
    role = db.execute("SELECT feedback FROM users WHERE username = :username",
...
@app.route("/index")
def indextemp():
    return render_template("indextempl.html")
...
def login():
    ...
    if request.method == "POST":
        ...
        if not input_username:
            return render_template("login-custom.html",messager = 1)



        # Ensure password was submitted
        elif not input_password:
             return render_template("login-custom.html",messager = 2)
        ...
        if len(username) != 1 or not check_password_hash(username[0]["hash"], input_password):
            return render_template("login-custom.html",messager = 3)

        # Remember which user has logged in
        session["user_id"] = username[0]["id"]
        session["username"] = username[0]["username"]
        session["feedback"] = username[0]["feedback"]
    ...
    # User reached route via GET (as by clicking a link or via redirect)
        ...
        return render_template("login-custom.html")
...
def register():
    ...
    if request.method == "POST":
        ...
        input_username = request.form.get("username")
        input_password = request.form.get("password")
        input_feedback = request.form.get("feedback")
        ...
        # Query database to insert new user
            ...
            new_user = db.execute("INSERT INTO users (username, hash, feedback) VALUES (:username, :password, :feedback)",
                                  username=input_username,
            ...
            session["user_id"] = new_user
            session["feedback"] = input_feedback
            # Flash info for the user
            flash(f"Registered as {input_username} / {input_feedback}")
